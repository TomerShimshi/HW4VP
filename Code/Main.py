...
new_video_stabilize.stabalize_video('Inputs/INPUT.avi','Outputs/stabilize_{}_{}.avi'.format(ID1,ID2))
...
new_bg_sub.background_subtraction('Outputs/stabilize_{}_{}.avi'.format(ID1,ID2))
end_bg = time.time()
time_to_bg_in_min= np.round((end_bg- start_all)/60)
print('time to finshe bg sub took {} minutes'.format(time_to_bg_in_min))

new_matting_v2.matting('Outputs/stabilize_203200480_320521461.avi','Outputs/binary_203200480_320521461.avi','Inputs/background.jpg')
end_mat = time.time()
time_to_mat_in_min= np.round((end_mat- start_all)/60)
print('time to finshe matting took {} minutes'.format(time_to_mat_in_min))

tracking.tracking('Outputs/matted_{}_{}.avi'.format(ID1,ID2),'Outputs/alpha_{}_{}.avi'.format(ID1,ID2))
end_track = time.time()
